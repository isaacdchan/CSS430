...
bool Shop::enterShop(int customer_id) {
    ...
    if (customer_queue.empty()) {
        // if not, check if a barber is free
        barber = findAvailableBarber();
        log("customer", customer_id, "queue is empty");
    }

    // if no barber is available, check the waiting line
    if (barber == NULL) {
        if (customer_queue.size() == num_waiting_chairs) { // waiting line is full
            log("customer", customer_id, "leaves the shop because of no available waiting chairs.");
            num_rejections++;
            pthread_mutex_unlock(&mutex);
            return false;
        } else {  // waiting line has space
            customer_queue.push(customer_id);
            log("customer", customer_id, "no barbers available. takes a waiting chair. seats available = " + intToString(waitingChairsAvailable()));
