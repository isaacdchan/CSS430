...
i32 fsRead(i32 fd, i32 numb, void* buf) {
    ...
    i32 currFbnStartingByte = startingFbn * BYTESPERBLOCK;
    ...
    if (startingFbn == lastRequestedFbn) {
      ...
      memcpy(buf, tempBuf + cursorBlockIndex, numb);
    ...
    for (i32 fbn = startingFbn; fbn <= lastRequestedFbn; fbn++) {
        ...
        if (fbn == startingFbn) { //fisrt block
          tempBufOffset = cursorBlockIndex;
          copySize = BYTESPERBLOCK - cursorBlockIndex;
        }
        if (fbn == lastRequestedFbn) { //last block
          copySize = (numb - (BYTESPERBLOCK - cursorBlockIndex)) % BYTESPERBLOCK;
          if(copySize == 0) {
            lastRequestedFbn--;
          }
        }
